...
export function reconcile(
  ...
  const forced = applyForcedDecisions(
    input.pos,
    input.settlement,
    request.decisions.accepted,
    request.decisions.manual,
    fingerprint
  )

  const rejectedPairs = new Set(
    request.decisions.rejected.map(pairKey)
  )

  const availablePos = input.pos.filter(
    (row) => !forced.usedPos.has(row.id)
  )

  const availableSettlement = input.settlement.filter(
    (row) => !forced.usedSettlement.has(row.id)
  )
  ...
  const usedPos = new Set(
    matched.map((match) => match.pos.id)
  )

  const usedSettlement = new Set(
    matched.map((match) => match.settlement.id)
  )
  ...
  const reviewPosIds = new Set(
    review.map((item) => item.pos.id)
  )

  const reviewSettlementIds = new Set(
    review.flatMap((item) =>
      item.candidates.map(
        (candidate) => candidate.settlement.id
      )
    )
  )

  const unmatched = {
    pos: input.pos.filter(
      (row) =>
        !usedPos.has(row.id) &&
        !reviewPosIds.has(row.id)
    ),

    settlement: input.settlement.filter(
      (row) =>
        !usedSettlement.has(row.id) &&
        !reviewSettlementIds.has(row.id)
    )
  }
...
function toAutoMatch(
  ...
  return {
    ...
    expectedSettlementAmount:
      candidate.expectedSettlementAmount,

    expectedSettlementTime:
      candidate.expectedSettlementTime,
...
function buildReviewItems(
  ...
  for (const [posId, bucket] of byPos) {
    ...
    const visible = bucket.slice(
      0,
      MATCH_CONFIG.maxReviewCandidates
    )
    ...
    items.push({
      ...
      whyNotAuto:
        top.reasons.length > 0
          ? top.reasons
          : [
              'Score did not clear the conservative automatic threshold.'
            ]
    })
  }
...
function reviewPriority(
  ...
  const nearTie =
    candidates.length > 1 &&
    Math.abs(
      top.score - secondBest
    ) < MATCH_CONFIG.minimumMargin

  if (
    top.duplicateConflict ||
    nearTie ||
    parseBdtToPoisha(
      top.pos.amount_bdt
    ) >= 200_000
