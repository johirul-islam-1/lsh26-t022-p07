    ...
    <div className="stack">
      <div className="table-wrap">
        <table className="data-table">
          <thead><tr><th>POS</th><th>Settlement</th><th>Confidence</th><th>Amount Δ</th><th>Time Δ</th><th>Type</th></tr></thead>
          <tbody>
            {matches.map((match) => {
              ...
              return (
                <tr key={key} onClick={() => setSelected(selected === key ? null : key)} className="clickable-row">
                  ...
                  <td><span className={`status-pill status-pill--${match.type.toLowerCase()}`}>{match.type}</span></td>
                </tr>
              )
            })}
          </tbody>
