...
export function MatchEvidence({ match }: { match: Match | null }) {
  ...
  return (
    <section className="evidence-card">
      <div className="panel__head"><div><span className="eyebrow">Why this pair</span><h3>{match.pos.id} ↔ {match.settlement.id}</h3></div><strong>{Math.round(match.confidence * 100)}%</strong></div>
      ...
      <div className="safety-list">
        {match.explanation.map((line) => <span key={line}>✓ {line}</span>)}
      </div>
    </section>
  )
}
